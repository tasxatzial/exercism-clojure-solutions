...
;; solution 1, (simpler, less demanding)
(defn composites
...
;; solution 2 (6 times faster, more demanding)
(defn delete-mult-k
...
(defn sieve2
  "Returns all primes in [2, max]."
  [max]
  (let [nums (vec (take (/ max 2) (range 3 (inc max) 2)))
        sqrt (Math/round (Math/floor (Math/sqrt max)))]
    (loop [i 3
           nums nums]
      (if (> i sqrt)
        (cons 2 (filter pos? nums))
        (if (pos? (get nums (/ (- i 3) 2)))
          (recur (+ i 2) (delete-mult-k i nums max))
          (recur (+ i 2) nums))))))
