...
(defn filter-positions-by-value
  [board positions value]
  (filter #(= value (:value (get board %))) positions))

(defn wins?
  [value board dimensions]
  (let [edge-positions-fn (value->edge-positions-fn value)
        {:keys [start-positions end-positions]} (edge-positions-fn dimensions)
        value-start-positions (filter-positions-by-value board start-positions value)]
    (any-path? board value-start-positions (set end-positions))))
